...
def load_all_documents_with_hash(pdf_paths, file_hash):
    ...
    for path in pdf_paths:
        ...
        if text.strip():
            ...
            st.warning(f"[생발] {path.name} 의 텍스트가 비어 있습니다.")
    return documents
    ...
    def expand(query: str) -> str:
        try:
            prompt = HumanMessage(content=safe_unicode(
                ...
                "PDF 용어와 표현을 반영하고, 필요한 부여도 추가해도 좋아. "
                f"질문: {query}"
            ))
            if "김보경" in query:
                return "회계감사 김보경 프로에 대해 알려줘"
...
st.set_page_config(page_title="삼성전기 종중노조 상담사", layout="centered", page_icon="🤖")
st.title("🤖 삼성전기 종중노조 상담사")
...
user_query = st.text_input("무엇이 궁금하시는가요?", placeholder="예: 집행부 구성은?")
if user_query.strip():
